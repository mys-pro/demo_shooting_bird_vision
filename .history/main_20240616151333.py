...
class viewFinder:
    def __init__ (self):
        ...
        self.rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)  # Đặt ở giữa màn hình
...
# Lớp đối tượng hình vuông clickable
class ClickableSquare:
    def __init__(self):
        self.size = 100
        self.rect = pygame.Rect((SCREEN_WIDTH - self.size) // 2, (SCREEN_HEIGHT - self.size) // 2, self.size, self.size)
...
def draw(SCREEN, background, view_finder, clickable_square):
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                if view_finder.is_clicked():
                    clickable_square = ClickableSquare()  # Tạo hình vuông khi ống ngắm được click
                elif clickable_square and clickable_square.is_clicked():
                    clickable_square = None  # Xóa hình vuông khi nó được click

        draw(SCREEN, background, view_finder, clickable_square)
