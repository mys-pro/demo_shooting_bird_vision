...
# Tạo chim
def create_bird(bird_surface):
    new_bird = bird_surface.get_rect(midtop = (-100, SCREEN_HEIGHT // 2))
    return new_bird
def move_bird(birds):
    for bird in birds:
        bird.centerx += 5
    return birds
def draw_bird(birds, bird_surface):
    for bird in birds:
...
# Vẽ hình ảnh
def draw(SCREEN, background, view_finder, bird_surface, bird_list):
    bg, bg_x, bg_y = background 
    SCREEN.blit(bg, (bg_x, bg_y))

    bird_list = move_bird(bird_list)
    draw_bird(bird_list, bird_surface)
...
def main(SCREEN):
    ...
    bird_surface = pygame.image.load('assets/images/birds/bird1/b-1.png').convert_alpha()
    bird_list = []
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == spawn_bird:
                bird_list.append(create_bird(bird_surface))

        draw(SCREEN, background, view_finder, bird_surface, bird_list)
