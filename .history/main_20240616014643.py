    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Giới hạn vị trí chuột trong cửa sổ
                if not SCREEN.get_rect().collidepoint(mouse_x, mouse_y):
                    # Nếu chuột ra ngoài cửa sổ thì đặt lại vị trí vào trong cửa sổ
                    new_x = max(SCREEN.get_rect().left, min(mouse_x, SCREEN.get_rect().right - 1))
                    new_y = max(SCREEN.get_rect().top, min(mouse_y, SCREEN.get_rect().bottom - 1))
