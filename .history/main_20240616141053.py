...
icon = pygame.image.load('assets/images/icon.png').convert_alpha() # Lấy ảnh icon
...
def load_background(folder_path, SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    for file_name in os.listdir(folder_path): # Duyệt qua tất cả các mục trong thư mục
        ...
        if os.path.isfile(full_path): # Kiểm tra xem đây có phải là file không
             backgrounds.append(pygame.image.load(full_path).convert_alpha()) # Nếu là file, thêm vào danh sách background
...
def main():
    cursor_img = pygame.image.load('assets/images/viewfinder.png').convert_alpha() # Tải hình ảnh kính ngắm
