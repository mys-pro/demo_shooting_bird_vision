...
def main():
    ...
    while running:
        ...
        if not screen_rect.collidepoint(mouse_x, mouse_y):
            # Nếu chuột ra khỏi khung hình, đưa nó trở lại
            if mouse_x < screen_rect.left:
                mouse_x = screen_rect.left
            elif mouse_x > screen_rect.right:
                mouse_x = screen_rect.right
            if mouse_y < screen_rect.top:
                mouse_y = screen_rect.top
            elif mouse_y > screen_rect.bottom:
                mouse_y = screen_rect.bottom
            pygame.mouse.set_pos(mouse_x, mouse_y)  # Đặt lại vị trí chuột
        pygame.draw.circle(SCREEN, (0, 0, 255), (mouse_x, mouse_y), 10)  # Vẽ con trỏ
