...
class viewFinder:
    def __init__ (self):
        ...
        self.rect.center = self.image.get_rect().center  # Đặt tâm của self.rect trùng với tâm của ảnh gốc
    
    def follow_mouse(self):
        self.rect.center = pygame.mouse.get_pos()
    
    def draw(self, screen):
        # Vẽ ảnh viewfinder lớn
        screen.blit(self.image, self.image.get_rect(center=self.rect.center).topleft)
        # Vẽ ảnh viewfinder nhỏ
        screen.blit(self.image_smaller, self.rect.topleft)

    def draw_rect(self, screen):
        pygame.draw.rect(screen, (255, 0, 0), self.rect, 2)  # Vẽ hình chữ nhật màu đỏ với độ dày 2 pixel
